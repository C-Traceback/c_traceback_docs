...
### `ctb_clear_context` <Badge text="Function" />
Clear the C Traceback context (including errors and call stack).
```c
void ctb_clear_context(void);
```
<Callout type="info">If you are writing a library, you can call this at API entry to clear any remaining context.</Callout>
...
<Callout type="info">
Always use [`TRY`](#try-) instead of `TRACE` when error handling matters.
...
<Callout type="info">
Always call [`ctb_check_error`](/docs/API/throwing_errors#ctb_check_error-) after `TRACE_BLOCK` when error handling matters.
