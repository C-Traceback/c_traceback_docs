...
### `ctb_dump_traceback_signal` <Badge text="Function" />
Dump the traceback to stderr on signal error.
```c
void ctb_dump_traceback_signal(ctb_sig);
```

<Callout type="info">You can use this to write your own signal handler.
As mentioned in [ctb_install_signal_handler](#ctb_install_signal_handler-),
it works 99% of the time but is slightly unsafe.</Callout>

#### Parameters
<ParamsTable 
  params={[
    { name: "ctb_error", type: "CTB_Error", desc: "The signal error type." },
  ]}
/>
