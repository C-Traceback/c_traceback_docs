...
If you are not sure how to compile a C project with our library.
check [Get Started](/docs).
...
### `LOG_WARNING_FMT` <Badge text="Function" />
However, we need to be careful for division by zero. While this could be an issue,
sometimes a simple warning is more suitable than throwing an error and terminating the program.

We can use the [`LOG_WARNING_FMT`](/docs/API/logging#log_warning_fmt-) API to log a warning
